...
class SBNGraph(BaseGraph):
    ...
    def from_string(self, input_string: str, is_single_line=False) -> SBNGraph:
        ...
        self.add_nodes_from(nodes)
        self._check_is_dag()
        in_edges = [x[1] for x in self.edges]
        to_edges = [x[0] for x in self.edges]
        self._check_is_dag()
    ...
    def _check_islotate_node(self):
        print(self.edges)
        in_edges = [x[1] for x in self.edges]
        to_edges = [x[0] for x in self.edges]
        isolate_node = [x for x in self.nodes if x not in in_edges+to_edges]
        print(isolate_node)
        if isolate_node:
            raise SBNError("There is an isolate node in the graph")
    @staticmethod
